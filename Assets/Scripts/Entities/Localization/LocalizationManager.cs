...
namespace Entities.Localization
{
    public class LocalizationManager : MonoBehaviour
    {
                ...
                if (key.Contains($"_{dialogueId}_"))
                {
                    deleteRequests.Add(new Request
                    {
                        DeleteDimension = new DeleteDimensionRequest
                        {
                            ...
                            {
                                SheetId = int.Parse(NameToGid[sheetName]),
                                Dimension = "ROWS",
                                StartIndex = i,
                                EndIndex = i + 1
                            }
        ...
        {
            foreach (var table in NameToGid.Keys)
                DeleteRows(graph.DialogueId, table);

            foreach (var node in graph.AllNodes.OfType<TextNode>())
            {
            ...
            foreach (var node in graph.AllNodes.OfType<ChoiceNode>())
            {
                int i = 0;
